...
// ── Monthly Memos ─────────────────────────────────────────────────────────────
const MEMOS_SHEET = 'Monthly_Memos';

export async function ensureMemosSheetExists() {
  const sheets = getSheets(true);
  const meta = await sheets.spreadsheets.get({ spreadsheetId: SPREADSHEET_ID });
  const exists = meta.data.sheets.some(s => s.properties.title === MEMOS_SHEET);
  if (!exists) {
    await sheets.spreadsheets.batchUpdate({
      spreadsheetId: SPREADSHEET_ID,
      requestBody: { requests: [{ addSheet: { properties: { title: MEMOS_SHEET } } }] },
    });
    await sheets.spreadsheets.values.update({
      spreadsheetId: SPREADSHEET_ID,
      range: `${MEMOS_SHEET}!A1:C1`,
      valueInputOption: 'RAW',
      requestBody: { values: [['year', 'month', 'content']] },
    });
  }
}

export async function getMonthlyMemo(year, month) {
  const sheets = getSheets();
  const res = await sheets.spreadsheets.values.get({
    spreadsheetId: SPREADSHEET_ID,
    range: `${MEMOS_SHEET}!A2:C`,
  });
  const rows = res.data.values || [];
  const row = rows.find(r => Number(r[0]) === year && Number(r[1]) === month);
  return row ? row[2] || '' : '';
}

export async function saveMonthlyMemo(year, month, content) {
  const sheets = getSheets(true);
  const all = await sheets.spreadsheets.values.get({
    spreadsheetId: SPREADSHEET_ID,
    range: `${MEMOS_SHEET}!A:B`,
  });
  const rows = all.data.values || [];
  const rowIndex = rows.findIndex((r, i) => i > 0 && Number(r[0]) === year && Number(r[1]) === month);
  const newRow = [year, month, content];
  if (rowIndex >= 0) {
    await sheets.spreadsheets.values.update({
      spreadsheetId: SPREADSHEET_ID,
      range: `${MEMOS_SHEET}!A${rowIndex + 1}:C${rowIndex + 1}`,
      valueInputOption: 'RAW',
...
// ── Monthly Targets ──────────────────────────────────────────────────────────
const TARGETS_SHEET = 'Monthly_Targets';

export async function ensureTargetsSheetExists() {
  const sheets = getSheets(true);
  const meta = await sheets.spreadsheets.get({ spreadsheetId: SPREADSHEET_ID });
  const exists = meta.data.sheets.some(s => s.properties.title === TARGETS_SHEET);
  if (!exists) {
    await sheets.spreadsheets.batchUpdate({
      spreadsheetId: SPREADSHEET_ID,
      requestBody: { requests: [{ addSheet: { properties: { title: TARGETS_SHEET } } }] },
    });
    await sheets.spreadsheets.values.update({
      spreadsheetId: SPREADSHEET_ID,
      range: `${TARGETS_SHEET}!A1:F1`,
      valueInputOption: 'RAW',
      requestBody: { values: [['year', 'month', 'country', 'target_bookings', 'target_avg_spend', 'memo']] },
    });
  }
}

export async function getTargets(year, month) {
  const sheets = getSheets();
  const res = await sheets.spreadsheets.values.get({
    spreadsheetId: SPREADSHEET_ID,
    range: `${TARGETS_SHEET}!A2:F`,
  });
  const rows = res.data.values || [];
  return rows
    .filter(r => Number(r[0]) === year && Number(r[1]) === month)
    .map(r => ({
      year: Number(r[0]),
      month: Number(r[1]),
      country: r[2] || '',
      targetBookings: Number(r[3]) || 0,
      targetAvgSpend: Number(r[4]) || 0,
      memo: r[5] || '',
    }));
}

export async function upsertTarget(year, month, country, targetBookings, targetAvgSpend, memo) {
  const sheets = getSheets(true);
  const all = await sheets.spreadsheets.values.get({
    spreadsheetId: SPREADSHEET_ID,
    range: `${TARGETS_SHEET}!A:C`,
  });
  const rows = all.data.values || [];
  const rowIndex = rows.findIndex(
    (r, i) => i > 0 && Number(r[0]) === year && Number(r[1]) === month && r[2] === country
  );
  const newRow = [year, month, country, targetBookings, targetAvgSpend, memo || ''];
  if (rowIndex >= 0) {
    await sheets.spreadsheets.values.update({
      spreadsheetId: SPREADSHEET_ID,
      range: `${TARGETS_SHEET}!A${rowIndex + 1}:F${rowIndex + 1}`,
      valueInputOption: 'RAW',
      requestBody: { values: [newRow] },
    });
  } else {
